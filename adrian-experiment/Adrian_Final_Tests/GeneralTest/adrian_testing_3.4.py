...
#---------------- Setting the game enviroment ----------------
    ...
    def main(game, enemy):
        ...
        while max(fitnessValues) < 100 and generationCounter < max_generations:
            ...
            plt.savefig("adrian-testing-3" + "-algorithm-" + str(algorithm) +"-enemy-"+ str(enemy)+".png")
            plt.show()
            # DataFrame
            df.loc[int(algorithm)]=[min(fits),max(fits),mean,std]
            df.to_csv("adrian-testing-3" + "-algorithm-" + str(algorithm) +"-enemy-"+ str(enemy)+".csv", index=False)
            print("Saving...")
